...
function parsePage() {
  ...
  suggestedDom.forEach((card) => {
    ...
    cardData.currency = card.querySelector('b').textContent.includes('₽') ? 'RUB' : 'Невозможно определить валюту!';
  ...
  reviewsDom.forEach((card) => {
    ...
    cardData.author = { avatar: card.querySelector('.author img').getAttribute('src'), name: card.querySelector('.author span').textContent };
  ...
  let meta = {
    title: document.querySelector('title')?.textContent.split(' ').slice(0, 2).join(' ') || null,
    ...
    opengraph: {
      title: document.querySelector("meta[property='og:title']")?.getAttribute('content').split(' ', 2).join(' ') || null,
  ...
  product = {
    ...
    name: document.querySelector('title')?.textContent.split(' ', 2).join(' ') || null,
    isLiked: document.querySelector('.like.active') ? true : false,
    tags: {
      category: document.querySelector('.green')?.textContent.split(',') || null,
    ...
    oldPrice: oldPrice,
    discount: discount,
    discountPercent: `${discountPercent.toFixed(2)}%` ? `${discountPercent.toFixed(2)}%` : '0%',
    currency: currency,
